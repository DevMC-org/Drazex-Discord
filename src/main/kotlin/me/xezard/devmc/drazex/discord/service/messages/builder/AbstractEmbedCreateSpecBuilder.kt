 ...
 */
package me.xezard.devmc.drazex.discord.service.messages.builder
...
abstract class AbstractEmbedCreateSpecBuilder<T> (
    ...
    fun create(
        ...
        EmbedCreateSpec.builder().apply {
            title?.let { title(messagesService.replace(it, replaces)) }
            description?.let { description(messagesService.replace(it, replaces)) }
            url?.let { url(it) }
            color?.let { color(messagesService.getColorFromString(it)) }
            image?.let { image(it) }
            thumbnail?.let { thumbnail(it) }
            timestamp?.let { timestamp(it) }
            footer?.let {
                val text = messagesService.replace(it.text, replaces)

                footer(EmbedCreateFields.Footer.of(text, it.iconUrl))
            }
            author?.let {
                val name = messagesService.replace(it.name, replaces)

                author(EmbedCreateFields.Author.of(name, it.url, it.iconUrl))
            }
            fields?.let { fields(it.map { properties ->
                val name = messagesService.replace(properties.name, replaces)
                val field = messagesService.replace(properties.value, replaces)
